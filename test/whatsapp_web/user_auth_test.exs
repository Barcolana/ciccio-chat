defmodule WhatsappWeb.UserAuthTest do
  use WhatsappWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Whatsapp.Accounts
  alias Whatsapp.Accounts.Scope
  alias WhatsappWeb.UserAuth

  import Whatsapp.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, WhatsappWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      WhatsappWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        WhatsappWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
